...
def get_vocab_embedding(filename, vocab_size, embedding_dim, name):
  tf.logging.info("Reading vocab_embedding of size (%d, %d) from {}".format(filename), vocab_size, embedding_dim)
  vocab_embedding = tf.get_variable(name, shape=(vocab_size, embedding_dim), initializer=tf.constant_initializer(np.load(filename)))
  # vocab_embedding = tf.Variable(np.load(filename), name = name, dtype=tf.float32)
  if vocab_embedding.get_shape() != (vocab_size, embedding_dim):
      raise ValueError("Wrong embedding file against the vocabrary: {}".format(filename))
