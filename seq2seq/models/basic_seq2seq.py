...
class BasicSeq2Seq(Seq2SeqModel):
  ...
  def decode(self, encoder_output, features, labels):
    ...
    # Keep decoder so that it can be accessed from outside
    ...
    if self.mode == tf.contrib.learn.ModeKeys.INFER:
      return self._decode_infer(decoder, self.bridge, encoder_output, features,
                                labels)
    else:
      return self._decode_train(decoder, self.bridge, encoder_output, features,
                                labels)
